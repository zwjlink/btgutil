...
func (cs *CoinSet) TotalValue() (value btgutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue btgutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue btgutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount btgutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue btgutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue btgutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue btgutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue btgutil.Amount, coins []Coin) (Coins, error) {
...
// btgutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() btgutil.Amount {
	return btgutil.Amount(c.txOut().Value)
}
