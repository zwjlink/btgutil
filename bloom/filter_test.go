...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := btgutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(btgutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := btgutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := btgutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
